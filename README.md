...
**[Latest release — v1.0.4](https://github.com/ab2024103-cmd/Morsecode/releases/tag/v1.0.4)**

| Platform | File | Size |
|---|---|---|
| Windows 10/11 | `MorseCode_1.0.4_x64-setup.exe` · `MorseCode_1.0.4_x64_en-US.msi` | 3.0 MiB · 4.0 MiB |
| Windows, **offline PC** | `MorseCode_1.0.4_x64-setup-offline-webview2.exe` (bundles the WebView2 runtime) | 208.9 MiB |
| macOS (Apple silicon) | `MorseCode_1.0.4_aarch64.dmg` | 3.5 MiB |
| macOS (Intel) | `MorseCode_1.0.4_x64.dmg` | 3.6 MiB |
| Linux | `MorseCode_1.0.4_amd64.deb` · `MorseCode_1.0.4_amd64.AppImage` | 4.1 MiB · 78.8 MiB |
...
That is a startup failure in a GUI-subsystem binary, which by design has no console to print to. Since **v1.0.4** MorseCode writes every startup stage to a log and shows a native error dialog instead of vanishing:
...
```
boot     MorseCode 1.0.4 · windows x86_64 · exe "C:\Program Files\MorseCode\MorseCode.exe"
...
- use **`MorseCode_1.0.4_x64-setup-offline-webview2.exe`** from the release — it carries the runtime inside the installer, so it works on a machine that has never been online.

Other causes the v1.0.4 build now survives rather than dying on: an unwritable config directory, a corrupt `history.sqlite` (falls back to an in-memory database), a tray icon that the shell refuses, and any panic in a background task (`panic = "unwind"`, so a failed discovery or transfer can no longer abort the process).
...
```bash
git tag -a v1.0.4 -m "MorseCode v1.0.4" && git push origin v1.0.4
```
